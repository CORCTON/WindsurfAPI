...
| `RESPONSE_STORE_TTL_MS` | `3600000` | **空闲**超时(1 小时),不是保留上界。比的是距上次访问的时间,而每次成功 `GET` 都会把它刷新 —— 所以周期性读取可以让一个条目无限存活。原文写"会话保留时长",读者会当成绝对上界,那是高估;这一列现在如实写它是空闲超时。字节与条数上限仍然独立约束总量 |
