...
发版口径:门禁 **3246 pass / 0 fail**(基线 v3.9.6 = 3119,新增 127 条)。

> 数字更正(2026-08-03 复核):此处原写「3245 pass,v3.9.7 基线 3195,新增 50 条」。
> 三个数字都不对。3245/3195 是撰写时中间分支的读数,被当成发布基线引用了;而 v3.9.7
> **从未打 tag**(它折进了本版发布),所以不存在「v3.9.7 基线」这回事。在 tag 上重测:
> `v3.9.6 = 3119`、`v3.9.8 = 3246`。

每条修复都做了突变验证 —— 把缺陷重新引入,确认断言真的会失败,而不是只看绿灯。这一轮
因此查出**我自己写的 4 条测试是空转的**,详见"工程"一节。
