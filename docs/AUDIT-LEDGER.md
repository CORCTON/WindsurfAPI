...
| 面 | 缺口 |
|---|---|
| ~~#240 预算耦合~~ | **已修**(本轮)。11 条突变 CAUGHT + 1 条有据可查的漏网 |
...
| #234 | 主体已修(`auth.js:358` 按 connect 后端切 `freeTierModels`,正是报告的错配),**但 issue 仍开着**。它的验收清单有六条,未逐条核 —— 关别人的 bug 报告之前要核完 |
| Cascade 流式路径不记 per-account 消费 | 既有,未修(同上一轮) |
| billing tag 号 | 需付费 token 校准(#239) |
