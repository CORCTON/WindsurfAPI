...
| `release.yml` 的 action 升级 | 九个 action 升到 node24(第十二轮)。**ci.yml 那部分合并即被真实运行验证,release.yml 那部分要等下一次打 tag** —— 届时若某个 action 出问题,表现是发版失败、产物不发布(可恢复,但那次运行会是红的) |
| #236 | 判据太模糊。正文只有"反代给 chatgpt、遇到浏览器操作就报错",无请求体、无错误码、无模型名。**该做的是向报告者要能区分候选机制的信息,不是从代码倒推一份猜测清单** |
| #235 | 技术主体 v3.9.8 已发(费率表判免费)。**但 `getCurrentlyFreeConnectSelectors` 只有一个生产调用者(`auth.js:322`,干旱判定内部),没有任何接口暴露它** —— 所以报告者(pro 号、设了 GLM-5-2 一下烧完周限)仍然看不见哪些模型吃配额。第十二轮的 dashboard `reachable` 标注部分缓解了"哪些调得动",但"哪些**免费**"仍未暴露 |
