...
1280 行、十二轮追加。**它是按时间追加的,不是按主题组织的** —— 同一个面会在多轮里出现,
后面的取代前面的。所以:

- **想知道当前哪些面没扫过** → `Ctrl-F` 搜 `仍未 exhaustive 扫描`,**取最后一个命中**。
  这个标题在文件里出现多次(第五、十、十一、十二轮各一次,加文件中部那张 2026-07-27 的
  原表),**后面的取代前面的**;原表自己开头也标了已被取代。
  *这一段的行数与轮次会随每轮追加而过期 —— 若与实际不符,以 `grep -c "^## 20"` 为准。*
