...
1100 多行、十一轮追加。**它是按时间追加的,不是按主题组织的** —— 同一个面会在多轮里出现,
后面的取代前面的。所以:

- **想知道当前哪些面没扫过** → `Ctrl-F` 搜 `仍未 exhaustive 扫描`,**取最后一个命中**
  (第十一轮)。这个标题在文件里出现四次(第五、十、十一轮各一次,加文件中部那张
  2026-07-27 的原表),**后面的取代前面的**;原表自己开头也标了已被取代
- **想知道某个子系统的结论** → 先看下面的分面小节(协议层 / 鉴权 / 图片解码),那三节从
  2026-07-27 至今没有被推翻;再用 `Ctrl-F` 搜模块名看后续轮次有没有补充
- **想知道方法论** → 每一轮的"判别动作"小节。最常复发的三条:
  修复只覆盖部分路径(已 6 次)、守卫守在了错误的一侧、探针坏了却当成被测对象没那个性质
...
| ~~Cascade **流式**路径不记 per-account 消费~~ | **已修**(第十二轮)。这一行原本写着"已加测试钉住" —— **那句是假的**,见第十二轮:全仓没有任何测试碰过这条路径的消费记账 |
| billing tag 号 | 需付费 token 校准(#239) |
...
| PR #241 | 已审完,三条回给贡献者:第 2 条理由在 connect 路径上不成立(`emulateTools` ⟺ `params.tools` 非空,两处字面同条件)、nudge 累积被 digest 放大 43 倍(242 → 10457 字符)、字段号 0 不可达但守卫在调用方。六条审查突变全 CAUGHT,测试质量高 |
...
**没有那个测试。** 把所有提到 `recordAccountSpend` 的测试文件逐个翻过 —— 没有一条碰流式
路径的消费记账。这句话是我自己在上一轮写下的。

这正是本仓库反复踩的那一类:**注释/文档声称有覆盖,而实际零断言**(第四轮记过两次、第七
轮记过一次)。区别在于这次载体是台账本身 —— 而台账是"哪些地方被守住了"的权威来源,所以它
说谎的代价比一句代码注释更高:下一个人会据此认为这条不需要再管。

**动作**:往台账写"已加测试钉住"时,把测试文件名和断言一起写进去。没有文件名的这句话
下一轮无法复核,而不可复核的声明在这个仓库里等于假的。
...
本轮之前 guard 6 只是理论上的。**"先提交,再突变"这条纪律在本轮第三次兑现**(第四、
七、八、九轮各记过一次)。
