...
> 把 WindsurfAPI 从创建到今天（2026-04-09 → 2026-08-11）的**每一件事**记下来：
...
*账本生成：2026-08-11。数据源：git log 全量 1205 条 + gh pr view 69 个 + gh issue 177 个 + 11 个并行采集 agent 的分片报告（/tmp/ledger-out-*.md）。所有 hash 可 git show 验证。*
