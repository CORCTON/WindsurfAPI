...
//
// `secret-scan` correctly flagged a full JWT literal here the first time it ran in CI
// (jwt-literal, this file). A fixture that looks exactly like a real credential is a real
// finding even when it is fake — the scanner cannot tell, and neither can the next reader.
// Assembling the parts keeps the shape the redactor must match while leaving nothing in
// the tree that scans as a secret.
