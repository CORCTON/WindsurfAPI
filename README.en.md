...
| `RESPONSE_STORE_TTL_MS` | `3600000` | An **idle** timeout (1 hour), not a retention ceiling. It measures time since last access, and every successful `GET` refreshes that — so periodic reads keep an entry alive indefinitely. This row used to say "conversation retention", which reads as an absolute bound and overstates it. The byte and count caps still bound the total independently. |
